...
<div class="redux-main">
    ...
    <?php foreach ($this->parent->sections as $k => $section): ?>
        <?php if (isset($section['customizer_only']) && true === $section['customizer_only']) continue; ?>

        <?php $section['class'] = isset($section['class']) ? ' ' . $section['class'] : ''; ?>
        <?php $disabled = '';
        if (isset($section['disabled']) && $section['disabled']) $disabled = 'disabled '; ?>
        ...
        <div
            ...
            <?php
				...
				if (
					isset($_GET['page'])
					&& $this->parent->args['page_slug'] === $_GET['page']
					&& isset($section['panel'])
					&& false === $section['panel']
				) {
					$display = false;
